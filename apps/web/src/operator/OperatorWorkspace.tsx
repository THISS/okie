...
export function OperatorWorkspace({ onPreview, initialRunId, initialDraftRevisionId }: { onPreview(runId: string, draftRevisionId: string, scopes: OperatorScope[]): Promise<void>; initialRunId?: string; initialDraftRevisionId?: string }) {
  ...
  useEffect(() => {
    ...
    void loadSelectedRun(selectedRun.runId).catch(cause => setError(cause instanceof Error ? cause.message : 'Could not refresh this run.'));
    return () => { requestEpoch.current += 1; };
  }, [selectedRun?.runId]);
  useEffect(() => {
    if (!selectedRun || !['queued', 'running'].includes(selectedRun.state)) return;
    const timer = window.setInterval(() => { void loadSelectedRun(selectedRun.runId).catch(cause => setError(cause instanceof Error ? cause.message : 'Could not refresh this run.')); void refreshRuns().catch(cause => setError(cause instanceof Error ? cause.message : 'Could not refresh runs.')); }, 5000);
  ...
  const mutateRevision = async <T,>(work: () => Promise<T>): Promise<T> => {
    try { return await work(); }
      ...
      if (cause instanceof OperatorApiError && cause.status === 409) await reloadDraft();
      throw cause;
    }
  };
  ...
  return <main className="operator-shell">
    ...
    <section className="operator-start"><h2>Scan a public repository</h2><form onSubmit={event => { event.preventDefault(); void act('start', async () => { const started = await operatorApi.start(url, crypto.randomUUID()); await refreshRuns(); setSelectedDraftRevisionId(undefined); setAcknowledged(false); setDetail(undefined); setScope(undefined); setRunActivity(undefined); setSelectedRun(started.run); setUrl(''); setNotice(started.deduped ? 'Reopened the matching active run.' : 'Scan queued.'); }); }}><input aria-label="Public GitHub repository URL" placeholder="https://github.com/owner/repository" required type="url" value={url} onChange={event => setUrl(event.target.value)}/><button disabled={busy === 'start'}>{busy === 'start' ? 'Starting…' : 'Start scan'}</button></form></section>
    <div className="operator-layout"><aside><h2>Runs</h2>{runs.length === 0 ? <p className="operator-muted">No scan runs yet.</p> : <ol className="operator-runs">{runs.map(run => <li key={run.runId}><button className={selectedRun?.runId === run.runId ? 'selected' : ''} onClick={() => { setSelectedDraftRevisionId(undefined); setAcknowledged(false); setDetail(undefined); setScope(undefined); setRunActivity(undefined); setSelectedRun(run); }}><strong>{run.source.owner}/{run.source.repo}</strong><span>{run.state} · {time(run.updatedAt)}</span></button></li>)}</ol>}</aside>
