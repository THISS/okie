  ...
  >
    ...
    <div className="brief-prose">
      ...
      <section data-testid="architecture-brief-containers">
        <h2>Containers ({brief.containers.length})</h2>
        ...
        {brief.containers.length === 0
          ...
          : brief.containers.slice(0, expanded.has('containers') ? undefined : 5).map(container => {
        ...
        {brief.containers.length > 5 && <button aria-expanded={expanded.has('containers')} onClick={() => toggle('containers')}>{expanded.has('containers') ? 'Show fewer' : `Show all ${brief.containers.length}`}</button>}
      ...
      <section data-testid="architecture-brief-flows">
        <h2>Key flows ({brief.flows.length})</h2>
        {brief.flowsMermaid
        ...
        {brief.flows.length === 0
          ...
          : <ul>{brief.flows.slice(0, expanded.has('flows') ? undefined : 5).map(flow => (
            <li data-brief-flow-id={flow.id} key={flow.id}>
              <strong>{flow.fromName}</strong> {flow.label} <strong>{flow.toName}</strong>
            </li>
          ))}</ul>}
        {brief.flows.length > 5 && <button aria-expanded={expanded.has('flows')} onClick={() => toggle('flows')}>{expanded.has('flows') ? 'Show fewer' : `Show all ${brief.flows.length}`}</button>}
