...
import { frameEntities, frameSemanticEntities, measuredStorySafeArea, readableRootCamera, storySafeArea, type SafeArea, type ViewportSize } from './storyFraming';
import {
...
const semanticDetails: SemanticDetail[] = ['context', 'container', 'component', 'code'];
...
function projectionScopeEntityIds(scene: AtlasScene, rootEntityId: string, detail: SemanticDetail): string[] {
...
function scopeFitsSafeViewport(
  scene: AtlasScene,
  rootEntityId: string,
  detail: SemanticDetail,
  camera: Camera,
...
type InspectorMorphBounds = { x: number; y: number; width: number; height: number };
        ...
        <section className="map-stage" aria-label="Architecture workspace">
          ...
          {currentStory ? (
            <section aria-label={`Guided architecture story, ${storyPlaybackStatus}`} className="story-player" data-playback-state={storyPhase === 'flight' ? 'moving' : storyPhase === 'arrival' ? 'arriving' : storyPlaying ? 'playing' : 'paused'} data-story-phase={storyPhase}>
              ...
              <div className="story-progress">{story.steps.map((step, index) => <button aria-label={`Go to story step ${index + 1}: ${step.title}`} className={index === storyStep ? 'active' : index < storyStep ? 'passed' : ''} key={step.title} onClick={() => setStep(index, false)}><span/></button>)}</div>
