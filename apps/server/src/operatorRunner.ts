  ...
  const runner: OperatorRunner = { async enqueue(input) {
    ...
    if (input.kind === "refresh") {
      ...
      for (const scopeId of [...new Set(input.scopeIds ?? [])].filter(scopeId => byId.get(scopeId)?.stale).sort((left, right) => depth(right) - depth(left) || left.localeCompare(right))) {
        await runner.enqueue({ ...input, kind: "retry", draftRevisionId: activeDraftRevisionId, scopeIds: [scopeId] });
    ...
    if (input.kind === "retry") {
      ...
      const adapter: OperatorEnrichmentStore = { async createAttempt(attempt) { attempts.set(attempt.attemptId, deps.store.createAttempt({ draftRevisionId: draft.draftRevisionId, scopeId: attempt.scopeId, kind: "retry", state: "running", modelId: attempt.modelId, inputHash: attempt.inputHash }).attemptId); }, async updateAttempt(id, patch) { const mapped = attempts.get(id); if (mapped) deps.store.updateAttempt(mapped, { state: patch.state === "accepted" ? "accepted" : patch.state === "cancelled" ? "cancelled" : "failed", ...(patch.usage ? { usage: persistedUsage(patch.usage)! } : {}), ...(patch.error ? { error: patch.error } : {}) }); }, async latestAttempt(scope) { const prior = sidecar.explanations.some(value => value.scopeId === scope) || acceptedContent.has(scope); return prior ? { attemptId: "sidecar", scopeId: scope, role: "owner", state: "accepted", modelId: "", inputHash: "", createdAt: 0, updatedAt: 0 } : undefined; }, async putAcceptedExplanation(scope, explanation, attempt, inputHash) { acceptedContent.set(scope, explanation); const row = deps.store.putAcceptedExplanation({ draftRevisionId: draft.draftRevisionId, scopeId: scope, attemptId: attempts.get(attempt)!, inputHash, content: explanation, validation: { accepted: true, validator: "operator-enrichment/v1" } }); acceptedVersionId.set(scope, row.explanationVersionId); return { explanationVersionId: row.explanationVersionId }; }, async getAcceptedExplanation(scope) { const prior = sidecar.explanations.find(value => value.scopeId === scope); return (acceptedContent.get(scope) ?? prior?.content) as OperatorExplanation | undefined; }, async markStale(ids) { ids.forEach(id => staleScopes.add(id)); } };
      ...
      const nextScopes = sidecar.scopes.map(scope => ({ ...scope, ...(staleScopes.has(scope.scopeId) ? { stale: true } : {}), ...(accepted && scope.scopeId === scopeId ? { stale: false } : {}) }));
