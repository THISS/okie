...
export function createOperatorRunner(deps: OperatorRunnerDeps): OperatorRunner {
  return { async enqueue(input) {
    ...
    if (input.kind === "retry") {
      ...
      const nextSidecar = { ...sidecar, scopes: sidecar.scopes.map(scope => ({ ...scope, ...(staleScopes.has(scope.scopeId) ? { stale: true } : {}) })), explanations: sidecar.explanations.map(value => value.scopeId === scopeId && accepted && acceptedContent ? { scopeId, content: acceptedContent, ...(acceptedVersionId ? { explanationVersionId: acceptedVersionId } : {}) } : value) };
      ...
      const nextDraft = deps.publication.createDraftRevision({ runId: run.runId, artifactRevisionId: nextArtifact.artifactRevisionId, coverage: { total: draft.coverage.total, accepted: draft.coverage.accepted, failed: draft.coverage.failed + (accepted ? 0 : 1), stale: [...staleScopes].length } });
