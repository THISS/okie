  ...
  const runner: OperatorRunner = { async enqueue(input) {
    ...
    if (input.kind === "retry") {
      ...
      deps.store.withExclusiveLock(() => {
        if (deps.store.snapshot().runs.find(value => value.runId === run.runId)?.draftRevisionId !== draft.draftRevisionId) return;
