...
export class OperatorStore {
  ...
  createRun(input: CreateRunInput): { run: OperatorRun; deduped: boolean } { idOk(input.idempotencyKey); idOk(input.source.repositoryId, true); idOk(input.source.slug); return this.ownerLock.with(() => { const state = this.load(); const prior = state.runs.find(run => run.idempotencyKey === input.idempotencyKey); if (prior) return { run: prior, deduped: true }; const at = this.now(); const run: OperatorRun = { runId: this.id("run"), idempotencyKey: input.idempotencyKey, source: { ...input.source }, state: "queued", createdAt: at, updatedAt: at }; state.runs.push(run); this.save(state); return { run, deduped: false }; }); }
