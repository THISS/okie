...
## Incremental rescan (commit-diff scoped)

Every scan is pinned to a commit (stage 1). A refresh pins a new commit and reuses everything the diff proves untouched — and must never produce different bytes than a from-scratch scan.

1. **Pin B, diff A→B.** `git diff --name-status A..B` with rename detection yields changed/added/deleted/renamed paths. A dependency-revision change (lockfile, submodule) marks its whole language scope dirty.
2. **Dirty-scope mapping.** Changed paths → extraction scopes (the fan-out units) → entities/claims whose `sourceRefs` resolve into those paths → transitive dependents via `derivation.inputClaimLogicalIds`. Everything else is clean.
3. **Reuse by content hash, not path trust.** A clean scope's prior extraction output is reused only when its source-content hash still matches; pure renames re-anchor `sourceRefs` without re-extraction. Path-diff selects candidates; content hashes decide.
4. **Merge to a full snapshot.** Snapshot B is complete and immutable; A is never mutated. Fingerprint matching computes `SnapshotLineage` (`unchanged|changed|new|no-longer-observed`) — history is marked, never erased.
5. **Enrichment carry-forward.** Explanations and spec sections carry forward when all supporting claim fingerprints and the prompt policy are unchanged (recorded as carried-forward), else they stale. Unchanged code consumes zero enrichment credits (cache domains).

**Equivalence gate.** For any A→B, the incremental path's canonical bytes and every semantic hash must equal a from-scratch scan of B. Corpus cases: rename-heavy diffs, dependency-only changes, delete+recreate, and shuffled diff order. This belongs as a new row in the `ingestion-golden-tests.md` golden fixture matrix when the incremental path lands — the optimization is only permitted because it is provably invisible.

## Milestones

- **M1 — deterministic TS extractor, dogfooded. CORE SHIPPED** (`@okie/scan`, see `scan-runner.md` R1): superset + discovery-order acceptance met; the *stretch* — retiring the hand-authored golden fixture — remains open. That stretch also retires a known identity wart (review finding): the hand-authored fixture's content can change (excerpt re-anchors, story edits) while `GOLDEN_WORKTREE_REVISION` stays constant — a violation of the identity contract that machine-produced, commit-pinned snapshots make impossible. *Acceptance:* the produced snapshot's entity/relation IDs are a superset of the hand-authored golden anchors in `golden-fixture.ts`; output is byte-identical across ≥100 shuffled discovery orders (Discovery-order + Platform + Unicode rows); per-file outline completeness — every exported/top-level symbol of a scanned file appears as a `code` entity with `path`+`symbol`+line anchors, so "what is in this file" is a grouped, `startLine`-sorted query (`get_file_outline` in [`mcp-surface.md`](./mcp-surface.md)), not new extraction. *Stretch:* the machine snapshot replaces the hand-authored fixture, folding source-excerpt regeneration into the scan and retiring the manual dogfooding pin (see [`deferred-refactors.md`](./deferred-refactors.md)).
- **M2 — rescan and diff.** Second scan at a new commit yields `SnapshotLineage` (schema doc), with the rescan commit-diff scoped per "Incremental rescan" above. *Acceptance:* the "Rescan unchanged" and "Rescan change" rows — unchanged facts keep fingerprints (only audit time differs); a changed source/value/rule flips only dependent fingerprints and stales dependents; unrelated claims untouched. Overrides from `materializeArchitectureAuthoring` survive and conflicts surface. The incremental path passes the equivalence gate (incremental ≡ full at B).
