...
test('CLA-118: 79 children pack 6–8 landscape columns, not a 3-col skyscraper', () => {
  ...
  // Closed-form seed is ~8; the ±1 refine can land on 7–9. All of those are
  // landscape packs, not the 3×27 skyscraper.
  assert.ok(columns >= 6 && columns <= 9, `landscape 79-pack must be ~6–8 columns, got ${columns}`);
  ...
  assert.ok(wide.rows <= 14, `landscape pack must cut rows vs 27, got ${wide.rows}`);
